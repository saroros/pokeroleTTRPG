...
export async function uploadFightHeaderImages(formData: FormData): Promise<void> {
  const schema = z.object({
    slot: z.enum(["1", "2"]),
  });
  ...
  if (!allowed.includes(file.type)) throw new Error("Only PNG or JPG allowed.");

  // limit size (e.g. 2 MB)
  const MAX = 2 * 1024 * 1024;
  if (file.size > MAX) throw new Error("Image too large (max 2MB).");
  ...
  await db.fightConfig.upsert({
    ...
    create:
      p.slot === "1"
        ? { id: "global", headerImageDataUrl: dataUrl }
        : { id: "global", headerImageDataUrl2: dataUrl },
    update:
      p.slot === "1"
        ? { headerImageDataUrl: dataUrl }
        : { headerImageDataUrl2: dataUrl },
  });

  revalidatePath("/fight");
}

export async function clearFightHeaderImage(formData: FormData): Promise<void> {
  const schema = z.object({
    slot: z.enum(["1", "2"]),
  });
  ...
  await db.fightConfig.upsert({
    ...
    create:
      p.slot === "1"
        ? { id: "global", headerImageDataUrl: "" }
        : { id: "global", headerImageDataUrl2: "" },
    update:
      p.slot === "1"
        ? { headerImageDataUrl: "" }
        : { headerImageDataUrl2: "" },
